...
log_msg <- paste("Starting GLoad2 with Logging set to ", log_loglevel)
info(logger, log_msg)
...
log_msg <- paste("Connecting to Database: ", graph_data_store)
debug(logger, log_msg)
gds <- dbConnect(RSQLite::SQLite(), graph_data_store)

create_gt <- function(gt_name) {
  # Create a graph master table that tells us which tables in the database are valid
  if(dbExistsTable(gds,mg_name)){
  log_msg <- paste("Table: ", gt_name, " already exists, replacing data")
  info(logger, log_msg)
  dbRemoveTable(gds, gt_name)
  # To-Do - put in the logic to merge new with old data - dbAppendTable(gds, tbl_name, nodes_data)
} else {
  log_msg <- paste("Table: ", gt_name , "is new, creating table")
  info(logger, log_msg)
}
...
# Firstly check that we have the two HR master tables. If they are not there, then we need to
# stop as we won't have any of the data we need
